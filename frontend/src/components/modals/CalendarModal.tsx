...
import { Calendar, X, Download, RefreshCw } from 'lucide-react';
...
/**
 * Modal de exportación de calendario.
 * Ofrece dos opciones: descarga ICS y sincronización directa con Google Calendar.
 */
const CalendarModal: React.FC<CalendarModalProps> = ({ isOpen, onClose, onConfirm, schedule }) => {
  const [syncing, setSyncing] = useState(false);
  const { isLoading: googleLoading, syncScheduleToCalendar } = useGoogleCalendar();

  if (!isOpen) return null;

  /** Calcula las fechas de inicio y fin del semestre automáticamente */
  const computeSemesterDates = (): { start: Date; end: Date } => {
  ...
  const handleIcsDownload = () => {
    const { start, end } = computeSemesterDates();
    onConfirm(start, end);
    onClose();
  };

  /** Sincroniza directamente con Google Calendar usando OAuth */
  const handleGoogleSync = async () => {
    ...
    try {
      ...
      alert('Error en la sincronización con Google Calendar.');
    } finally {
      setSyncing(false);
    }
  };
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-3">
            <button 
              ...
              disabled={syncing || googleLoading}
